    ...
    <ExperiencePage
      ...
      renderCard={(parade) => {
        const ParadeCardAction = () => {
          ...
          useEffect(() => {
            setIsAdded(readItinerary(email).some((entry) => entry.category === "parades" && entry.id === parade.id));
          }, [email]);

          const toggleItinerary = () => {
            if (isAdded) {
              removeExperienceFromItinerary("parades", parade.id, email);
            } else {
              addExperienceToItinerary("parades", parade.id, parade.name, email);
            }
